...
const zend_function_entry ext_functions[] = {
	PHP_FE(confirm_ext_compiled,	NULL)		/* For testing, remove later. */
...
{
	ext_globals->global_value = 0;
	ext_globals->global_string = NULL;
}
*/
/* }}} */
...
/* {{{ proto string confirm_ext_compiled(string arg)
   Return a string to confirm that the module is compiled in */
PHP_FUNCTION(confirm_ext_compiled)
{
	char *arg = NULL;
	int arg_len, len;
	char *strg;
	...
	len = spprintf(&strg, 0, "Congratulations! You have successfully modified ext/%.78s/config.m4. Module %.78s is now compiled into PHP.", "ext", arg);
